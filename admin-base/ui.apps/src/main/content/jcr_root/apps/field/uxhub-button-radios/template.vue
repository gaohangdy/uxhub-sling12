	...
	<div class="wrap">
		<ul v-if="!schema.preview" :class="schema.inline ? 'radio-list inline' : 'radio-list'" :disabled="disabled" :id="getFieldID(schema)">
		  <li v-for="item in items" :class="isItemChecked(item) ? 'checked' : ''">
			...
			<a :for="getItemName(item)+_uid" :title="getItemName(item)" class="waves-effect waves-light btn btn-small" @click="onSelection(item)" >
				<i v-if="item.icon" class="material-icons left">{{item.icon}}</i>
				<template v-if="schema.showItemLabel">
				{{ getItemName(item) }}
				</template>
			</a>
		  </li>
